...
# 프로젝트 루트의 config 폴더 경로를 자동으로 설정합니다.
# 현재 파일(kis_auth.py) 경로
file_path = os.path.dirname(__file__)
# config 경로
config_path = os.path.join(file_path, '..', 'config')
# config 경로 절대 경로 변환
config_root = os.path.abspath(config_path) + os.sep
...
token_tmp = config_root + 'KIS' + datetime.today().strftime("%Y%m%d") + '_' + _cfg['environment']  # 토큰 로컬저장시 파일명 년월일_환경
...
def auth(svr=None, product=_cfg['my_prod'], url=None):
    # svr이 없으면 환경파일에서 가져오기
    if svr is None:
        svr = _cfg['environment']
    print(f"\nauth 호출 - 실행 환경(svr): {svr}")
...
def reAuth(svr='prod', product=_cfg['my_prod']):
    ...
    if svr is None:
        svr = _cfg['environment']
    print(f"\reAuth 호출 - 실행 환경(svr): {svr}")
...
def _url_fetch(api_url, ptr_id, tr_cont, params, appendHeaders=None, postFlag=False, hashFlag=True):
    print(f"\n_url_fetch 호출 - api_url: {api_url}, ptr_id: {ptr_id}, tr_cont: {tr_cont}, params: {params}")
